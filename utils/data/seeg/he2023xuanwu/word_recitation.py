...
def load_subj_bipolar_default(path_subj, ch_names=None, use_align=True):
    ...
    for run_i in os.listdir(path_task):
        ...
        path_run_dataset_i = os.path.join(path_run_i, "dataset.bipolar.default.{}".format(
            ("aligned" if use_align else "unaligned")))
...
def load_run_bipolar_default(path_run, ch_names=None, use_align=True):
    ...
    path_run_dataset = os.path.join(path_run,
                                    "dataset.bipolar.default.{}".format(("aligned" if use_align else "unaligned")))
    # Return the final `X` & `y`.
...
def _load_subj_bipolar(path_run_datasets, ch_names=None):
    ...
    pattern_subj = re.compile(r"{}(\d+){}".format(re.escape(os.sep), re.escape(os.sep)))
    ...
    for path_run_dataset_i in path_run_datasets:
        ...
        dataset_data_i = load_pickle(path_run_dataset_data_i);
        np.random.shuffle(dataset_data_i)
        dataset_info_i = load_pickle(path_run_dataset_info_i)
        # Update `dataset_data` & `dataset_info`.
        dataset_data.append(dataset_data_i);
        dataset_info.append(dataset_info_i)
    ...
    label_sets = [set([data_i.name for data_i in dataset_data_i]) for dataset_data_i in dataset_data];
    labels = set()
...
def _load_run_bipolar(path_run_dataset, ch_names=None):
    ...
    dataset_data = load_pickle(path_run_dataset_data);
    ...
    labels = [data_i.name for data_i in dataset_data];
