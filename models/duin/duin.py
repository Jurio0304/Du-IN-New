...
class duin_vqvae(nn.Module):
    ...
    def _init_model(self):
        ...
        self.emb_time = TimeEmbedding(d_model=self.params.encoder.d_model, max_len=self.params.encoder.emb_len,
                                      mode="sincos")
    ...
    def _loss_rgs(self, value, target, weight=None):
        ...
        loss = torch.mean(
            torch.flatten(torch.permute(loss, dims=[0, -1, *range(1, len(loss.shape) - 1)]), start_dim=2, end_dim=-1),
            dim=-1)
...
class duin_mae(nn.Module):
    ...
    def forward(self, inputs):
        ...
        E_masked = [
            (E_init * (1. - mask[..., None].to(dtype=E_init.dtype)) + mask_emb * mask[..., None].to(
                dtype=E_init.dtype)),
            (E_init * mask[..., None].to(dtype=E_init.dtype) + mask_emb * (
                    1. - mask[..., None].to(dtype=E_init.dtype))),
        ]
        # Forward encoder block to get time-aligned token sequence.
    ...
    def gen_mask(self, E, mask_ratio=0.5):
        ...
        shuffle_idxs = torch.argsort(noise, dim=-1)
        restore_idxs = torch.argsort(shuffle_idxs, dim=-1)
        # Generate the bool mask: `False` is keep, `True` is remove.
        # mask - (batch_size, emb_len)
        mask = torch.ones((batch_size, emb_len), dtype=torch.bool).to(device=E.device)
        mask[:, :keep_len] = False
...
class duin_cls(nn.Module):
    ...
    def _init_model(self):
        ...
        self.contra_block = ContrastiveBlock(d_model=self.params.contra.d_model,
                                             d_contra=self.params.contra.d_contra,
                                             loss_mode=self.params.contra.loss_mode)
        # Initialize classification block.
    ...
    # def load_weight func
    def load_weight(self, path_ckpt, device=None):
        """
        Load model weights from the specified checkpoint path.

        Args:
            path_ckpt: str - The path of the spcified checkpoint.
            device: torch.device - The device to load the checkpoint
...
class duin_llm(nn.Module):
    ...
    def _init_model(self):
        ...
        cls_initials_params = cp.deepcopy(self.params.cls)
        cls_initials_params.n_tokens = cls_initials_params.n_initials
        cls_finals_params = cp.deepcopy(self.params.cls)
        cls_finals_params.n_tokens = cls_finals_params.n_finals
...
if __name__ == "__main__":
    ...
    assert seq_len % duin_vqvae_params_inst.model.seg_len == 0
    duin_vqvae_params_inst.model.seq_len = seq_len
    ...
    subj_id = torch.tensor(np.eye(n_subjects)[np.random.randint(0, n_subjects, size=(batch_size,))],
                           dtype=torch.float32)
    channel_mask = torch.ones((batch_size, n_channels), dtype=torch.bool)
    # Instantiate duin_vqvae.
    duin_vqvae_inst = duin_vqvae(duin_vqvae_params_inst.model)
    print(duin_vqvae_inst)
    ...
    assert seq_len % duin_mae_params_inst.model.seg_len == 0
    duin_mae_params_inst.model.seq_len = seq_len
    ...
    # X - (batch_size, seq_len, n_channels) c_true - (batch_size, emb_len, codex_size) subj_id - (batch_size, n_subjects)
    emb_len = token_len
    codex_size = duin_mae_params_inst.model.vq.codex_size
    X = torch.rand((batch_size, seq_len, n_channels), dtype=torch.float32)
    c_true = torch.tensor(np.eye(codex_size)[np.random.randint(0, codex_size, size=(batch_size, emb_len))],
                          dtype=torch.float32)
    subj_id = torch.tensor(np.eye(n_subjects)[np.random.randint(0, n_subjects, size=(batch_size,))],
                           dtype=torch.float32)
    # Instantiate duin_mae.
    duin_mae_inst = duin_mae(duin_mae_params_inst.model)
    print(duin_mae_inst)
    ...
    assert seq_len % duin_cls_params_inst.model.seg_len == 0
    duin_cls_params_inst.model.seq_len = seq_len
    ...
    duin_cls_inst = duin_cls(duin_cls_params_inst.model)
    print(duin_cls_inst)
    ...
    assert seq_len % duin_llm_params_inst.model.seg_len == 0
    duin_llm_params_inst.model.seq_len = seq_len
    ...
    duin_llm_inst = duin_llm(duin_llm_params_inst.model)
    print(duin_llm_inst)
